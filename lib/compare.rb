...
module Compare
  ...
  def initialize(dir1, dir2, task, txt)
    @dir1 = dir1
    @dir2 = dir2
    json = JSON.parse(txt)
  ...
  def vefiry_exists(dir)
    abort("#{dir} is not a directory") unless (Dir.exists?(dir))
  end
  ...
  def main(task, json)
    data = {}
    keys = nil # ???
    table = create_table(data, task, json)
...
# of unsupported tests          2583
  ...
  def create_table(data, task, json)
...
#  Dir.children(dir1).sort.each do |d|
    #  file1 = "#{dir1}/#{task}/gcc.sum"
    #  file2 = "#{dir2}/#{task}/gcc.sum"
  ...
  #
      ...
      results1 = read_results(get_file(@dir1))
      results2 = read_results(get_file(@dir2))
      tmp = []
