...
{   "test01": {     "tool1": "This is a version",     "tool2": "This is a version"   },   "test02": {     "tool3": "This is a version",     "tool4": "This is a version"   } }
EOF
)
...
cat <<'EOF' > config/config.json
{
	"tasks": {
		"test01": {
		 ...
		 "test02": {
			"execute": [
				"ruby $var(@CONFIG_SOURCE_PATH)/versions.rb tool3 > version1.json",
				"ruby $var(@CONFIG_SOURCE_PATH)/versions.rb tool4 > version2.json"
			],
			"publish_header": [
				"cat $var(@WORKSPACE)/version1.json",
				"cat $var(@WORKSPACE)/version2.json"
